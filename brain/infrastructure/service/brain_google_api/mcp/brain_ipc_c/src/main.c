...
static void build_agent_ids(char *agent_name, size_t agent_name_sz, char *agent_id, size_t agent_id_sz) {
  ...
  if (session && session[0] && strncmp(session, "agent_", 6) == 0) {
    name = session;
  }
  ...
  if (pane && pane[0]) {
    ...
    snprintf(agent_id, agent_id_sz, "%s", name);
  }
}
...
int main(int argc, char **argv) {
  ...
  McpJsonRpc mcp;
  mcp_jsonrpc_init(&mcp, stdout, &out_mu, "brain-ipc-c", "0.1");
  ...
  /* Auto-register */
  {
    ...
    if (pane && pane[0]) {
      ...
      json_t *data = json_object();
      json_object_set_new(data, "agent_name", json_string(agent_name));
      json_object_set_new(data, "metadata", json_pack("{s:s,s:b}", "mcp_type", "mcp-brain_ipc_c", "auto_registered", 1));
      json_object_set_new(data, "tmux_pane", json_string(pane));
      if (session && session[0]) json_object_set_new(data, "tmux_session", json_string(session));
      ...
      json_t *data = json_object();
      json_object_set_new(data, "service_name", json_string("mcp-brain_ipc_c"));
  ...
  while (!g_shutdown && fgets(line, sizeof(line), stdin)) {
    ...
    if (strcmp(method, "initialize") == 0) {
      ...
      json_t *info = json_pack("{s:s,s:s}", "name", "brain-ipc-c", "version", "0.1");
      json_t *res = json_object();
