...
def _content_blocks_from_assistant_message(
...
def _assistant_message_to_sse_events(
    assistant_message: Dict[str, Any],
    result_event: Optional[Dict[str, Any]],
    request_model: str,
) -> list[bytes]:
    content = _content_blocks_from_assistant_message(assistant_message, result_event)
    ...
    events: list[bytes] = [_sse_bytes("message_start", {"type": "message_start", "message": message})]

    for index, block in enumerate(content):
        ctype = str(block.get("type", "") or "")
        if ctype == "text":
            ...
            events.append(_sse_bytes("content_block_stop", {"type": "content_block_stop", "index": index}))
        else:
            events.append(
                _sse_bytes(
                    "content_block_start",
...
class ClaudeCLIProvider(BaseProvider):
    ...
    async def forward_stream(self, request: Dict[str, Any], protocol: str = "messages"):
        ...
        prompt_stdin.write(prompt.encode("utf-8"))
        prompt_stdin.flush()
        prompt_stdin.seek(0)
        ...
        async def _iter():
            ...
            if emitted == 0 and assistant_message is not None:
                for event_bytes in _assistant_message_to_sse_events(
                    assistant_message,
                    result_event,
                    str(request.get("model", model) or model),
                ):
                    emitted += 1
                    yield event_bytes
                return
