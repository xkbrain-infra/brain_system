...
static Queue* find_or_create_queue(MsgQueue *mq, const char *agent) {
    ...
    for (int i = 0; i < mq->queue_count; i++) {
        if (strcmp(mq->queues[i].agent, agent) == 0) {
            mq->queues[i].last_access = now;
            return &mq->queues[i];
        }
    }

    // Create new - find a free slot or evict LRU empty queue
    if (mq->queue_count >= MAX_QUEUES) {
        // LRU eviction: find oldest empty queue (no queued, inflight, or deadletter messages)
        int evict_idx = -1;
        time_t oldest_access = now + 1;

        for (int i = 0; i < mq->queue_count; i++) {
            Queue *q = &mq->queues[i];
            if (q->size == 0 && q->inflight_count == 0 && q->deadletter_count == 0) {
                if (evict_idx < 0 || q->last_access < oldest_access) {
                    oldest_access = q->last_access;
                    evict_idx = i;
                }
            }
        }
        ...
        if (evict_idx < 0) {
            // All queues have active messages - cannot evict
            fprintf(stderr, "[WARN] msgqueue: MAX_QUEUES=%d reached, all queues have active "
                    "messages, cannot create queue for agent '%s'\n", MAX_QUEUES, agent);
            return NULL;
        }

        fprintf(stderr, "[WARN] msgqueue: LRU evicting queue '%s' (last_access=%ld) to make "
                "room for new agent '%s'\n",
                mq->queues[evict_idx].agent, (long)oldest_access, agent);

        // Reuse the evicted slot (queue_count stays the same)
        Queue *q = &mq->queues[evict_idx];
