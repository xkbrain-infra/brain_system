...
锁定后需重启 agent 生效:
```bash
agentctl restart agent-xkquant_architect
```
...
# 3. 重启该 agent
agentctl restart agent-system_dev1
...
jq '.hooks.PreToolUse[0].hooks[0].env' \
  /brain/groups/org/brain_system/agents/agent-system_dev1/.claude/settings.local.json
```
