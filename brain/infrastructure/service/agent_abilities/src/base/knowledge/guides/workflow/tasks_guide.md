...
> 适用于 S6 Tasks 阶段的任务拆分与执行规划作战手册。Owner: **orchestrator** 主导，**architect** + **pmo** 参与。
...
### Phase 1: 规划（architect + pmo + orchestrator 共同完成）

1. **architect 拆任务**: 基于 05_solution.yaml 的模块列表，按分桶模板拆成原子任务
2. **orchestrator 制定执行计划**: 分批、关键路径、并行轨道、里程碑
3. **三方讨论 team_allocation**: 需要多少 dev agent、多少 qa agent

### Phase 2: 分配与执行（orchestrator 主导）

1. orchestrator 通过 agentctl 创建 dev/qa agent
...
team_allocation:
  discussion: "architect + pmo + orchestrator 共同决定"
  dev_count: 3
  qa_count: 1
...
state_machine:
  ...
  transitions:
    pending → assigned: "orchestrator 分配"
...
ipc_events:
  ...
  TASK_PROGRESS: "agent → orchestrator"
  TASK_COMPLETED: "agent → orchestrator → review/"
  TASK_BLOCKED: "agent → orchestrator"
  TASK_REVIEW_RESULT: "qa → orchestrator"
  TASK_AVAILABLE: "orchestrator → idle agent"
```
...
| 没有 deps 导致并行冲突 | 必须声明依赖，orchestrator 按拓扑排序分配 |
...
| orchestrator 不跟进 | timer 定期 ping，不能等 agent 主动汇报 |
