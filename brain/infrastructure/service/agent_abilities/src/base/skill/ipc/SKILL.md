...
# 严重错误
ipc_send(to="service-telegram_api", message="[ALERT] agent_system_dev 不可恢复错误\n错误: 数据库连接失败\n已停止执行，等待人工介入", priority="critical")
```
...
```
ipc_send(to="agent_system_frontdesk", message="[REPLY] 回复内容")
```
...
/ipc send agent_system_pmo [REPORT] 任务已完成
/ipc recv
/ipc search pmo
/ipc wait 120
