...
def check_spec_path(file_path: str, rules_yaml: Path = None) -> Tuple[bool, str]:
    ...
    # 1. 检查是否在禁止列表（仅对潜在 SPEC 文档生效）
    # G-SPEC-LOCATION 只管控 SPEC 文档，不拦截普通文件（如 .txt, .log 等）
    is_forbidden_path, matched_pattern = is_forbidden(file_path, forbidden_patterns)

    if is_forbidden_path:
        # 判断是否是潜在 SPEC 文档（yaml/yml/md 或路径含 spec）
        file_ext = Path(file_path).suffix.lower()
        is_potential_spec_doc = (
            file_ext in {'.yaml', '.yml', '.md'}
            or "/spec/" in file_path
            or "/specs/" in file_path
            or "spec" in Path(file_path).name.lower()
        )
        if not is_potential_spec_doc:
            # 非 SPEC 文档文件，跳过 forbidden 检查
...
"""
    ...
    # 2. 检查是否在允许列表（仅对 spec 相关文件生效）
    # 判断是否是 spec 文件：包含 "/spec/", "/specs/", 或文件名包含 "spec"
    is_spec_file = (
        "/spec/" in file_path
        or "/specs/" in file_path
        or "spec" in Path(file_path).name.lower()
    ...
    if allowed_patterns and is_spec_file:
