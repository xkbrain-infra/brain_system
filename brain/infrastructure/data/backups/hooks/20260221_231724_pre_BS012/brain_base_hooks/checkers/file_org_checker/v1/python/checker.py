...
def check_meaningless_names(file_path: str) -> Tuple[bool, str]:
    """检查是否包含无意义的目录名"""
    meaningless = ["misc", "other", "temp", "test", "untitled", "dump", "stuff"]
...
def check_path_depth(file_path: str, max_depth: int = 10) -> Tuple[bool, str]:
    """检查路径深度"""
    depth = file_path.count("/")
    if depth > max_depth:
        return False, f"路径过深（{depth} 层），建议不超过 {max_depth} 层"
    return True, ""
...
def check_file_organization(file_path: str) -> Tuple[bool, str]:
    ...
    # 4. 路径深度限制
    is_valid, error_msg = check_path_depth(file_path)
    if not is_valid:
        return False, error_msg
