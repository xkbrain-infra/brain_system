...
class BinaryChecker(BaseChecker):
    ...
    def check(self, context: CheckContext) -> CheckResult:
        ...
        target = context.file_path or context.command
...
class PathChecker(BaseChecker):
    ...
    def __init__(self, hook_root: Path = None):
        ...
        # Import path_checker (直接在该版本目录下)
        checker_path = self.hook_root / "checkers" / "path_checker" / "v1" / "python"
        ...
        # Fallback rules file
        self.rules_yaml = hook_root / "src" / "hooks" / "rules" / "spec_path.yaml"
...
class FileOrgChecker(BaseChecker):
    ...
    def __init__(self, hook_root: Path = None):
        ...
        try:
            ...
            checker_path = self.hook_root / "checkers" / "file_org_checker" / "v1" / "python" / "checker.py"
