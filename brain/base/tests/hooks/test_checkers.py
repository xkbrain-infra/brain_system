...
class TestNawpChecker:
    """Test G-GATE-NAWP protected-area detection"""

    def test_nawp_warns_on_live_brain_platform_without_plan(self):
        """Live /brain/platform paths should be treated as protected areas"""
        checker = NawpChecker()
        context = CheckContext(
            gate_id="G-GATE-NAWP",
            tool_name="Write",
            tool_input={"file_path": "/brain/platform/docker/compose.yaml"},
            enforcement={"warn_message": "warn {file_path}"},
            command=None,
            file_path="/brain/platform/docker/compose.yaml",
        )

        with patch.object(checker, "_has_plan", return_value=False):
            result = checker.check(context)

        assert result.status == CheckStatus.WARN
        assert result.gate_id == "G-GATE-NAWP"

    def test_nawp_warns_on_workspace_live_brain_platform_without_plan(self):
        """Workspace paths under /xkagent_infra/brain should also be protected"""
        checker = NawpChecker()
        context = CheckContext(
            gate_id="G-GATE-NAWP",
            tool_name="Write",
            tool_input={"file_path": "/xkagent_infra/brain/platform/docker/compose.yaml"},
            enforcement={"warn_message": "warn {file_path}"},
            command=None,
            file_path="/xkagent_infra/brain/platform/docker/compose.yaml",
        )
    ...
    def test_nawp_ignores_groups_source_paths(self):
        """Source paths outside /brain/ should not be treated as protected areas"""
        checker = NawpChecker()
        context = CheckContext(
            gate_id="G-GATE-NAWP",
            tool_name="Write",
            tool_input={"file_path": "/xkagent_infra/groups/brain/projects/platform/testing/index.yaml"},
            enforcement={"warn_message": "warn {file_path}"},
            command=None,
            file_path="/xkagent_infra/groups/brain/projects/platform/testing/index.yaml",
        )
