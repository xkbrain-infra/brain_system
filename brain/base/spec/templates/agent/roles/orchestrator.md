...
```yaml
  ...
  pre_spawn_checklist:
    ...
    - "若消息未显式指定 provider/model，则 sandbox worker 默认使用 --agent-type minimax --model minimax-m2.7"
    - "禁止把 anthropic 当成 agent-type 直接写入 agentctl；Claude 系列统一使用 --agent-type claude，模型用 claude-sonnet-4.6 / claude-opus-4.6 / claude-haiku-4.5"
