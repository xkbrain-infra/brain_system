...
DOMAINS=("config" "evolution" "hooks" "knowledge" "mcp" "platform" "scripts" "skill" "spec" "workflow")
...
usage() {
  cat <<'EOF'
Usage:
  publish_base.sh --dry-run [--domain config|evolution|hooks|knowledge|mcp|platform|scripts|skill|spec|workflow|root|agents|all]
  publish_base.sh --publish [--domain config|evolution|hooks|knowledge|mcp|platform|scripts|skill|spec|workflow|root|agents|all]

Domains:
  config   Publish config/ to brain/base/config
  ...
  platform Publish platform/ to brain/base/platform
...
case "$DOMAIN" in
  config|root|evolution|hooks|knowledge|mcp|platform|scripts|skill|spec|workflow|sandbox|agents|all)
    ;;
  *)
