...
规范: /xkagent_infra/groups/brain/platform/sandbox/index.yaml
"""
...
class SandboxConfig:
    ...
    def load_platform_config(cls) -> Dict:
        """加载平台主配置"""
        config_path = cls.ROOT / "index.yaml"
    ...
    def load_provider_config(cls, provider: str = "docker") -> Dict:
        """加载 Provider 配置"""
        config_path = cls.ROOT / "providers" / f"{provider}.yaml"
        with open(config_path) as f:
            return yaml.safe_load(f)
...
class SandboxManager:
    ...
    def _generate_compose(self, project: str, dep_type: str, instance_id: str,
        ...
        # 写入临时 compose 文件
        compose_file = self.config.ROOT / "instances" / f"{instance_id}.yaml"
