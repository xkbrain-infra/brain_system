...
metadata:
  status: active
  source_project: /xkagent_infra/groups/brain/platform/sandbox
  publish_target: /xkagent_infra/brain/base/skill/sandbox
  spec_ref: /xkagent_infra/groups/brain/platform/sandbox/index.yaml
  gates_ref: /xkagent_infra/groups/brain/platform/sandbox/gates.yaml
---
...
```bash
SANDBOX_ROOT="/xkagent_infra/groups/brain/platform/sandbox"
SANDBOX_SERVICE="python3 ${SANDBOX_ROOT}/service/sandbox_service.py"
```
...
|------|----------|----------|
| 平台配置 | `spec_ref` | 创建前 |
| LEP Gates | `gates_ref` | 验证时 |
| Docker Provider | `${SANDBOX_ROOT}/providers/docker.yaml` | 创建时 |
| Compose 模板 | `${SANDBOX_ROOT}/templates/compose.base.yaml` | 创建时 |
...
详细规则参见：`${SANDBOX_ROOT}/PROJECT_EXTENSION.md`
