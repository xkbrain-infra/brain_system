...
从 `brain/system` 镜像手动部署 Sandbox 容器：
...
|----------|----------|------|
| 9022 | 22 | SSH |
| 9080-9090 | 8080-8090 | Dashboard / Web 服务 |
| 9210 | 8210 | Agent 服务 |
| 9765 | 8765 | 监控/日志服务 |
...
# 创建并启动 sandbox 容器
docker run -d \
  --name brain-sandbox-test \
  --hostname brain-sandbox-test \
  -p 9022:22 \
  -p 9080-9090:8080-8090 \
  -p 9210:8210 \
  -p 9765:8765 \
  -v /var/run/docker.sock:/var/run/docker.sock \
  -v /xkagent_infra/runtime:/xkagent_infra/runtime:shared \
  -v /xkagent_infra/app:/app:shared \
  -v /xkagent_infra/groups:/groups:shared \
  brain/system:latest

# 查看容器状态
docker ps -f name=brain-sandbox-test
...
1. **SSH 服务**: 容器内 supervisor 自动管理 sshd
2. **共享卷**: 必须挂载以下目录用于持久化
   - `/xkagent_infra/runtime`: 运行时配置和日志
   - `/xkagent_infra/app`: 应用目录
   - `/xkagent_infra/groups`: 项目目录
3. **Docker sock**: 如需容器内管理 Docker
