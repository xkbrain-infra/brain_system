...
name: brain-publish
description: "当任务涉及发布 projects/base 变更、新增或修改 skill、scripts、mcp、将 skill 部署到 agent、或刷新 agent 运行态配置时使用。"
user-invocable: true
disable-model-invocation: false
allowed-tools: Read, Write, Edit, Bash, Glob
argument-hint: "[skill|spec|hooks|knowledge|workflow|scripts|mcp|agents|runtime|all|pending]"
metadata:
  status: active
...
| 情境 | 跳转 |
|------|------|
| 修改了 skill / spec / hooks / knowledge / workflow / scripts / mcp | → [发布 base 域](#发布-base-域) |
...
2. **改动是什么？**
   - skill / spec / hooks / knowledge / workflow / scripts / mcp → [发布 base 域](#发布-base-域)
...
`<domain>` 可选值：`skill` `spec` `hooks` `knowledge` `workflow` `scripts` `mcp` `evolution` `agents` `all`
