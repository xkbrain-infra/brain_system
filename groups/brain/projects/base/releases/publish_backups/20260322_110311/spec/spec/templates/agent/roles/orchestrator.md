...
```yaml
      ...
      - /brain/base/workflow/project_delivery/models/project_agent_provisioning.yaml
      - /brain/base/workflow/project_delivery/contracts/project_agent_runtime_creation.yaml
...
phase_playbook:
  ...
  BOOTSTRAPPING:
    ...
    done_when: "BOOTSTRAP_PASS 通过 → 项目进入 PLANNING"
  ...
  TASK_MODELING:
    ...
    steps:
      ...
      2: "等待 architect 输出 task_graph.yaml 到 /workspace/project/spec/06_tasks/task_graph.yaml"
      3: |
          确认文件存在后，调用 seeding 脚本：
          Bash("python3 /brain/base/workflow/project_delivery/implementation/seed_task_graph.py \
               --task-graph /workspace/project/spec/06_tasks/task_graph.yaml")
