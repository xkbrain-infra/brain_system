...
# Live publish entrypoint still operates on the canonical source tree.
...
ROOT_FILES=("index.yaml" "INIT.md.new")
DOMAINS=("evolution" "hooks" "knowledge" "mcp" "skill" "spec" "workflow")
...
usage() {
  cat <<'EOF'
Usage:
  publish_base.sh --dry-run [--domain evolution|hooks|knowledge|mcp|skill|spec|workflow|root|agents|all]
  publish_base.sh --publish [--domain evolution|hooks|knowledge|mcp|skill|spec|workflow|root|agents|all]
...
case "$DOMAIN" in
  root|evolution|hooks|knowledge|mcp|skill|spec|workflow|sandbox|agents|all)
    ;;
  *)
