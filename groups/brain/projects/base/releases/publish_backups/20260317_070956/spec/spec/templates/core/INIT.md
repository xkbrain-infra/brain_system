...
## 7. 核心 LEP 规则（需主动遵守）
以下规则为Agent决策阶段需主动遵守的核心规则，**其他29个LEP规则已由系统底层（Hooks/Daemon/Skill）自动强制执行，违规操作会被直接拦截，无需主动记忆**：

| Gate | Rule |
|------|------|
| G-GATE-SCOPE-DEVIATION | 禁止静默缩减执行范围，做不到要明确说明并通知PMO |
| G-GATE-ATOMIC | Plan 必须原子化（具体到文件/行号/动作），禁止模糊描述 |
| G-GATE-APPROVAL-DELEGATION | 需要审批时发送给PMO，禁止直接询问用户 |
| G-GATE-DEFER | 产生延迟任务/待办事项时必须主动通知PMO |
| G-GATE-UNRECOVERABLE | 遇到不可恢复错误立即停止执行并上报PMO |

**约束冲突时**：① 拒绝执行 → ② 说明违反了哪个 Gate → ③ 提供正确方式

完整规则定义：`/brain/base/spec/core/lep.yaml`
