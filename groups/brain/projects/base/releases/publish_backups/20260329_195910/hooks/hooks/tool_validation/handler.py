...
def _normalize_result_for_tool(tool_name: str, result: Optional[CheckResult]) -> Optional[CheckResult]:
    """Normalize LEP outcomes for specific tool classes.

    Policy:
      - Read-only tools must never be denied by LEP. They may warn, but must pass.
      - Stateful tools (Bash/Write/Edit/...) keep the original deny/warn outcome.
    """
    if result is None:
        return None

    if tool_name in _READ_ONLY_TOOLS and result.is_block:
        message = (result.message or "").rstrip()
        if message:
            message += "\n\n"
        message += (
            "NOTE: Read/Grep/Glob are downgraded to warning-only. "
            "This operation is allowed to proceed."
        )
        return CheckResult.warn(
            result.gate_id or "G-READ-ONLY-ALLOW",
            message,
            result.priority,
        )
...
def handle_pre_tool_use():
    ...
    try:
        ...
        override_result = _normalize_result_for_tool(
            tool_name,
            _run_overrides(tool_name, tool_input),
        )
        ...
        engine_result = _normalize_result_for_tool(
            tool_name,
            engine.check(tool_name, tool_input),
        )
