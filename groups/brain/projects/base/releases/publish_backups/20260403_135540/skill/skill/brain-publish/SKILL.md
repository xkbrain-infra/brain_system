...
| **发布 brain-docker 镜像** | → [发布 brain-docker](#发布-brain-docker) |
...
- `publish_base.sh` 内部会回退到 `/xkagent_infra/groups/brain/projects/base` 作为 source_root，这是预期行为
...
Step 5  $PUBLISH_BASE --dry-run --domain agents   ← 确认 stale skill 清单
...
> 深入规则：`/brain/base/spec/policies/deployment/docker_publish.yaml`

brain-docker 遵循**三域架构**：Source → Runtime → Published
...
# Step 2: 构建验证（dry-run 模式）
bash scripts/build.sh --dry-run

# Step 3: 执行构建
bash scripts/build.sh

# Step 4: 同步到 Published 域
# 自动同步（由 build.sh 完成）:
# - Dockerfile → $DOCKER_PUB/
# - compose.yaml → $DOCKER_PUB/
# - configs/* → $DOCKER_PUB/configs/
# - scripts/* → $DOCKER_PUB/scripts/

# Step 5: 验证 Published 域
ls -la $DOCKER_PUB/
```
...
**版本升级规则：**
- PATCH (1.0.0 → 1.0.1): Bug 修复，配置调整
- MINOR (1.0.0 → 1.1.0): 新增功能，向后兼容
- MAJOR (1.0.0 → 2.0.0): 重大变更，可能破坏兼容性

### 镜像标签策略

```bash
# 构建时自动打标签
brain/docker-base:latest
brain/docker-base:1.0.0
brain/docker-base:1.0
brain/docker-base:1

# 特定版本锁定
brain/docker-base:1.0.0-20260315
```

### 发布后验证

- [ ] `docker images | grep brain/docker-base` 显示新镜像
- [ ] Published 域文件已更新 (`/xkagent_infra/brain/platform/docker/`)
- [ ] compose.yaml 可直接运行 (`docker compose up -d`)
- [ ] SSH 服务可访问 (`ssh root@localhost -p 8622`)
- [ ] 容器内 AI CLI 工具可用 (`claude --version`)

### 回滚流程

```bash
DOCKER_PUB=/xkagent_infra/brain/platform/docker
BACKUP_DIR=/xkagent_infra/app/brain/docker/backups

# 1. 从备份恢复 Published 域
cp $BACKUP_DIR/<version>/Dockerfile $DOCKER_PUB/
cp $BACKUP_DIR/<version>/compose.yaml $DOCKER_PUB/

# 2. 重建镜像
cd $DOCKER_PUB
docker build -t brain/docker-base:<version> .
