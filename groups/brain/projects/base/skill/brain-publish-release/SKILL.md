---
id: G-SKILL-DEPLOY-BRAIN-PUBLISH-RELEASE
name: brain-publish-release
...
metadata:
  ...
  publish_target: /xkagent_infra/brain/base/skill/brain-publish-release
---

# brain-publish-release
...
| 需要同步整体版本号与 image 版本号 | → 使用本 skill 决策版本，再交给 `/brain-publish-image` |
| 只是 Brain 内部发布链改动进入运行态 | → 使用 `/brain-publish` |
| 只是构建 docker image / push registry | → 使用 `/brain-publish-image` |
...
当前 source 内未发现独立的 `brain-publish-release` spec。
