...
#define LOG_FILE_DEFAULT "/xkagent_infra/runtime/logs/brain_ipc.log"
...
static int find_pty_for_pid(pid_t start_pid, char *buf, size_t sz) {
    ...
    LOG_INFO("find_pty_for_pid: no PTY found (walked up from pid=%d)", (int)start_pid);
    return 0;
}
...
static char* handle_service_register(json_t *data, int client_fd) {
    ...
    // Store pty_path if provided or auto-discovered (for non-tmux push notifications)
...
static int parse_source_filter(const char *source, int default_filter, int *out_filter) {
    ...
    if (strcasecmp(source, "all") == 0) {
        *out_filter = REGISTRY_SOURCE_FILTER_ALL;
        return 0;
    }
    if (strcasecmp(source, "agent") == 0 || strcasecmp(source, "agents") == 0) {
        *out_filter = REGISTRY_SOURCE_FILTER_AGENT;
        return 0;
    }
    if (strcasecmp(source, "service") == 0 || strcasecmp(source, "services") == 0) {
        *out_filter = REGISTRY_SOURCE_FILTER_SERVICE;
        return 0;
    }
    if (strcasecmp(source, "register") == 0) {
        *out_filter = AGENT_SOURCE_REGISTER;
        return 0;
    }
    if (strcasecmp(source, "heartbeat") == 0) {
        *out_filter = AGENT_SOURCE_HEARTBEAT;
        return 0;
    }
    if (strcasecmp(source, "tmux_discovery") == 0 || strcasecmp(source, "discovery") == 0) {
        *out_filter = AGENT_SOURCE_TMUX_DISCOVERY;
        return 0;
    }
    return -1;
}
...
static char* handle_service_list(json_t *data) {
    ...
    char services_json[16384];
    int count = registry_list_filtered(
        &g_registry,
        services_json,
        sizeof(services_json),
        include_offline != 0,
        REGISTRY_SOURCE_FILTER_SERVICE
    );
    ...
    snprintf(
        buf,
        BUFFER_SIZE,
        "{\"status\":\"ok\",\"services\":%s,\"count\":%d,\"include_offline\":%s}\n",
        services_json,
        count,
        include_offline ? "true" : "false"
    );
    return buf;
}

static char* handle_registry_search(json_t *data) {
    const char *query = json_string_value(json_object_get(data, "query"));
    if (!query) query = "";
    ...
    int fuzzy = 1;
    json_t *fuzzy_val = json_object_get(data, "fuzzy");
    if (fuzzy_val && json_is_false(fuzzy_val)) {
        fuzzy = 0;
    }

    int limit = 50;
    json_t *limit_val = json_object_get(data, "limit");
    if (limit_val && json_is_integer(limit_val)) {
        limit = (int)json_integer_value(limit_val);
    }
    if (limit <= 0) limit = 50;
    if (limit > 512) limit = 512;

    int source_filter = REGISTRY_SOURCE_FILTER_ALL;
    const char *source = json_string_value(json_object_get(data, "source"));
    if (parse_source_filter(source, REGISTRY_SOURCE_FILTER_ALL, &source_filter) != 0) {
        return json_error("invalid source filter (use all/agent/service/register/heartbeat/tmux_discovery)");
    }

    char results_json[32768];
    int count = registry_search(
        &g_registry,
        query,
        fuzzy != 0,
        include_offline != 0,
    ...
    snprintf(
        buf,
        BUFFER_SIZE,
        "{\"status\":\"ok\",\"results\":%s,\"count\":%d,\"fuzzy\":%s,\"include_offline\":%s,\"limit\":%d}\n",
        results_json,
        count,
        fuzzy ? "true" : "false",
        include_offline ? "true" : "false",
...
// ============ PTY Push Notification (non-tmux fallback) ============

static void pty_notify(const char *pty_path, const char *from, const char *msg_id) {
    if (!pty_path || !pty_path[0]) return;

    int fd = open(pty_path, O_WRONLY | O_NONBLOCK);
    if (fd < 0) {
        LOG_WARN("pty_notify: open(%s) failed: %s", pty_path, strerror(errno));
        return;
    }
    char msg[512];
...
// Unified notification: tmux_pane > pty_path > skip
static void notify_agent(const Agent *agent, const char *from, const char *msg_id) {
    ...
    } else if (agent->pty_path[0]) {
        pty_notify(agent->pty_path, from, msg_id);
    }
}
...
static char* handle_request(const char *req_str, int client_fd) {
    ...
    } else if (strcmp(action, "service_list") == 0) {
        response = handle_service_list(data);
    ...
    } else if (strcmp(action, "registry_search") == 0 || strcmp(action, "search") == 0) {
        response = handle_registry_search(data);
...
static void* conversation_cleanup_thread(void *arg) {
    ...
    while (!g_shutdown) {
        ...
        if (!g_shutdown) {
            int evicted = registry_evict_stale(&g_registry);
            if (evicted > 0) {
                LOG_INFO("Evicted %d stale registry entries (offline > %ds)", evicted, EVICT_TIMEOUT);
            }
